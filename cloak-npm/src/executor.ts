...
export interface ExecutionParams { [key: string]: string | number | boolean | null | undefined; }
export interface ExecutionResult { success: boolean; status?: number; data?: unknown; error?: string; }

export const SERVICE_ACTIONS: Record<string, Array<{ id: string; label: string; params: string[] }>> = {
  github: [
...
type ServiceHandler = (credential: string, action: string, params: ExecutionParams) => Promise<ExecutionResult>;

async function githubHandler(token: string, action: string, params: ExecutionParams): Promise<ExecutionResult> {
  const base = "https://api.github.com";
  const headers: Record<string, string> = {
    Authorization: `Bearer ${token}`, Accept: "application/vnd.github+json",
    "X-GitHub-Api-Version": "2022-11-28", "Content-Type": "application/json", "User-Agent": "Cloak-Vault/1.0",
  };
  let url = "", method = "GET";
  ...
  switch (action) {
    ...
    case "create_issue": {
      const owner = String(params["owner"] ?? ""), repo = String(params["repo"] ?? "");
      if (!owner || !repo) return { success: false, error: "owner and repo required" };
      url = `${base}/repos/${owner}/${repo}/issues`; method = "POST";
...
async function slackHandler(token: string, action: string, params: ExecutionParams): Promise<ExecutionResult> {
  const base = "https://slack.com/api";
  const headers: Record<string, string> = { Authorization: `Bearer ${token}`, "Content-Type": "application/json" };
  let url = "", method = "GET";
  let body: Record<string, unknown> | undefined;
  switch (action) {
    case "post_message": url = `${base}/chat.postMessage`; method = "POST"; body = { channel: params["channel"], text: params["text"] }; break;
...
async function stripeHandler(apiKey: string, action: string, params: ExecutionParams): Promise<ExecutionResult> {
  const base = "https://api.stripe.com/v1";
  const headers: Record<string, string> = { Authorization: "Basic " + Buffer.from(apiKey + ":").toString("base64"), "Content-Type": "application/x-www-form-urlencoded" };
  let url = "";
  switch (action) {
...
async function veniceHandler(apiKey: string, action: string, params: ExecutionParams): Promise<ExecutionResult> {
  ...
  switch (action) {
    case "chat": {
      ...
      return { success: res.ok, status: res.status, data: { response, model: data.model ?? model } };
    }
    case "image_gen": {
      const prompt = String(params["prompt"] ?? "");
...
async function selfHandler(appScope: string, action: string, params: ExecutionParams): Promise<ExecutionResult> {
...
const SERVICE_HANDLERS: Record<string, ServiceHandler> = {
  github: githubHandler, slack: slackHandler, stripe: stripeHandler,
  venice: veniceHandler, self: selfHandler,
};

export class ExecutionEngine {
  constructor(private vault: CredentialVault) {}

  async execute(service: string, action: string, params: ExecutionParams): Promise<ExecutionResult> {
    let credential: string;
    try { credential = this.vault.retrieve(service); }
    catch { return { success: false, error: `No credential found for: "${service}"` }; }
    try {
      const handler = SERVICE_HANDLERS[service.toLowerCase()];
      if (!handler) return { success: false, error: `No handler for: "${service}". Supported: ${Object.keys(SERVICE_HANDLERS).join(", ")}` };
